...
We'll be exploring various plot types (i.e., geometric objects), only using the `x` and `y` aesthetics (and `group`).
...
On your worksheet, fill out the grammar components for a scatterplot of `gdpPercap` vs. `lifeExp`. We'll ignore the "coordinate system" and "facetting" components after this.
