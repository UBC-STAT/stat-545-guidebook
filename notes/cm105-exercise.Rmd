---
title: "cm105 and cm106 Notes and Exercises"
subtitle: "Write your own R package -- Lecture Version"
date: 'November 6 and 8, 2017'
output: 
    html_document:
        toc: true
---

This tutorial aims to get you started with package development in R. It draws on [Jenny Bryan's STAT 547 tutorial](http://stat545.com/packages06_foofactors-package.html), focussing more exclusively on the RStudio workflow to building packages. Instead of building her [`foofactors`](https://github.com/jennybc/foofactors) package, we'll be building the [`powers`](https://github.com/vincenzocoia/powers) package.
...
Install the following packages to aid in package development:
...
Comment package functions with `#'` above the function, and use tags starting with `@`. Let's document the `square` function.
...
Make and document another function, say `cube`, that raises a vector to the power of 3. Be sure to `@export` it to the NAMESPACE.

Finished early? Do more -- work on Assignment 9, and/or try out more documentation features that comes with `roxygen2` (the `@` tags).
...
As a template, save the following script in a file called `test_square`, filling in the blanks with an `expect` statement:
