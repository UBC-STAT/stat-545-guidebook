...
In our R exploration, we saw one way to bundle together multiple pieces of information: _vectors_. Now, we'll turn to _data frames_.
...
Let's explore the gapminder dataset by continuing to work on the R Markdown document we started earlier.

1. What _is_ a data frame? Minimum need-to know: tabular data.
    - Load the gapminder package with the `library` function, and print `gapminder` to screen.
2. Explore a data frame with things like `ncol`, `str`, `summary`, `head`.
3. Extract a column with `$`. Maybe compute its variance.
