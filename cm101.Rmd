...
- (20%) Assignments: These will be short assignments designed to help you practice material from the current week

- (60%) Team projects: There will be two team projects in this course (each equally weighted), one building an analysis pipeline and the second creating a dashboard using DashR. You will stay with the same groups for the duration of the course. Though the projects are separate, they will use the same dataset and should be considered as extensions of each other.

- (10%) Teamwork: The teamwork document will be due after the end of each project and is your opportunity to reflect on your group dynamics and share with the teaching team your successess and challenges. It will constitute a 4 point rating scale on a few categories as well as an area for written feedback. A template teamwork document will be provided to you. It is not intended for this to be a long written exercise (unless you choose to do so, we will read it all). 
...
There will be a "milestone" assignment due each week where you will be expected to work with your partner collaboratively in a single GitHub repository using the workflows you learned in STAT 545.
...
Keep in mind that you will be working with the dataset you choose for the remainder of the course, so here are some guidelines for choosing a dataset:

- Make sure you have some basic interest in the subject matter
- Ensure there are over 5000 rows/observations in the dataset
- Ensure there are at least 5 "columns of potential interest" in the dataset
...
In R, there's a handy package called `glue`.
...
break the line the way the tidyverse style guide recommends to make the operations easier to spot, and order by `lo` in descending order:
...
1. First, clone your `participation` repo for STAT 547 and navigate to the `week01` folder. Create a new subfolder inside the `week01` folder called `data`.

2. At the end of the `cm101_participation.Rmd` file, load the `here` package (recall it is used to construct paths without specifying user-specific paths).
...
3. Download the [person.csv](https://github.com/gvwilson/tidynomicon/raw/master/data/person.csv) file into the `data` folder you created. 
